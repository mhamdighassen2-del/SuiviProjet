...
export const projetsService = {
    ...
    importExcel: (
        fichier: File,
        meta: { nom: string; client: string; date_debut?: string; date_fin_prevue?: string }
    ) => {
        const form = new FormData();
        form.append('fichier', fichier);
        form.append('nom', meta.nom);
        form.append('client', meta.client);
        if (meta.date_debut) form.append('date_debut', meta.date_debut);
        if (meta.date_fin_prevue) form.append('date_fin_prevue', meta.date_fin_prevue);
        return api
            .post<{ data: ImportExcelResult; message: string }>('/projets/import-excel', form, {
