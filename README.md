...
| `004_ai_conversations.sql` | *(Historique)* Créait la table `ai_conversation` — **ne plus enchaîner** dans les nouveaux setups ; la 005 supprime cette table. |
| `005_drop_ai_conversation.sql` | Supprime la table `ai_conversation` si elle existe (retrait du module assistant IA). |
...
npm install
npm run db:setup
# Inclut les migrations 001 → 003, puis 005 (suppression table IA si présente), puis seed.
```

**Pourquoi `db:setup` ?** Crée la base si besoin, enchaîne migrations `001` → `003`, puis `005`, puis seed.  
Équivalent manuel : `npm run db:create` puis `db:migrate`, `db:migrate:002`, `db:migrate:003`, `db:migrate:005`, `db:seed` (voir `backend/package.json`). Si votre base avait encore l’ancienne table IA, `005` la supprime.
