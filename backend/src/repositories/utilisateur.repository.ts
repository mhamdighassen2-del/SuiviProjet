...
import { CelluleType, CreateUtilisateurDTO, RoleUtilisateur, UpdateUtilisateurDTO } from '../models/types';
...
export const UtilisateurRepository = {
    async findAll(): Promise<UtilisateurRow[]> {
        const { rows } = await db.query<UtilisateurRow>(
            `SELECT u.id, u.nom, u.prenom, u.email, u.role::text AS role, u.actif, u.cree_le, u.service_id,
                    s.nom::text AS service_nom, u.cellule::text AS cellule
             FROM utilisateur u
    ...
    async create(dto: CreateUtilisateurDTO, mot_de_passe_hash: string): Promise<UtilisateurRow> {
        const { rows } = await db.query<UtilisateurRow>(
            `INSERT INTO utilisateur (nom, prenom, email, mot_de_passe_hash, role, service_id, cellule)
             VALUES ($1, $2, $3, $4, $5::role_utilisateur, $6, $7::cellule_type)
             RETURNING id, nom, prenom, email, role::text AS role, actif, cree_le, service_id, cellule::text AS cellule`,
            [dto.nom, dto.prenom, dto.email.trim(), mot_de_passe_hash, dto.role, dto.service_id ?? null, dto.cellule ?? null]
        );
    ...
    async update(id: string, dto: UpdateUtilisateurDTO, mot_de_passe_hash?: string | null): Promise<UtilisateurRow | null> {
        ...
        if (dto.nom !== undefined) { fields.push(`nom = $${i++}`); values.push(dto.nom); }
        if (dto.prenom !== undefined) { fields.push(`prenom = $${i++}`); values.push(dto.prenom); }
        if (dto.email !== undefined) { fields.push(`email = $${i++}`); values.push(dto.email.trim()); }
        if (mot_de_passe_hash !== undefined && mot_de_passe_hash !== null) {
            fields.push(`mot_de_passe_hash = $${i++}`); values.push(mot_de_passe_hash);
        }
        if (dto.role !== undefined) { fields.push(`role = $${i++}::role_utilisateur`); values.push(dto.role); }
        if (dto.actif !== undefined) { fields.push(`actif = $${i++}`); values.push(dto.actif); }
        if (dto.service_id !== undefined) { fields.push(`service_id = $${i++}`); values.push(dto.service_id); }
        if (dto.cellule !== undefined) { fields.push(`cellule = $${i++}::cellule_type`); values.push(dto.cellule); }

        if (!fields.length) return UtilisateurRepository.findById(id);

        values.push(id);
        const { rows } = await db.query<UtilisateurRow>(
            `UPDATE utilisateur SET ${fields.join(', ')} WHERE id = $${i} RETURNING id`,
            values
        );
    ...
    async countProjetsResponsable(id: string): Promise<number> {
        const { rows } = await db.query<{ c: string }>(
            `SELECT COUNT(*)::int AS c FROM projet WHERE responsable_id = $1`, [id]
        );
