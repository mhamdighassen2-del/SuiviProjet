...
function toApiUser(row: UtilisateurRow) {
    return {
        ...
        cellule: (row.cellule as CelluleType | undefined) ?? undefined,
    };
}
...
export const UtilisateurService = {
    ...
    async modifier(id: string, dto: UpdateUtilisateurDTO) {
        ...
        const nextServiceId = dto.service_id !== undefined ? dto.service_id : current.service_id;
        ...
        if (dto.email && dto.email.trim().toLowerCase() !== current.email.toLowerCase()) {
            ...
            if (taken && taken.id !== id) throw new Error('Cet email est déjà utilisé');
        }
    ...
    async supprimer(id: string, currentUserId: string) {
        ...
        if (row.role === 'ADMIN') {
            ...
            if (admins <= 1) throw new Error('Impossible de supprimer le dernier administrateur.');
        }
